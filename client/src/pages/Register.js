...
export default function Register() {
  ...
  useEffect(() => { if (preId) setValue("activityId", String(preId)); }, [preId, setValue]);

  useEffect(() => {
    api.get("/activities/summary").then((res) => setActivities(res.data || [])).catch(() => setActivities([]));
  }, []);
  ...
  const onSubmit = async (values) => {
    ...
    try {
      const payload = {
        ...
        email: values.email,
        phone: values.phone,
        school: internal ? "" : (values.school || ""),
        activityId: Number(values.activityId),
      };
      const { data } = await api.post("/registrations", payload);
      setResp(data);
      setMsg({ ok: true, text: "✅ ¡Inscripción creada! Revisa tu correo o guarda tu QR." });
  ...
  const fieldCls = "mt-1 block w-full rounded-xl border-slate-300 focus:border-umgBlue focus:ring-umgBlue";
    ...
    <div className="min-h-screen bg-umgBlue text-white">
      <div className="max-w-xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        <div className={cardCls}>
          ...
          <p className="text-slate-600">Completa tus datos y elige una actividad disponible.</p>
          ...
          <form onSubmit={handleSubmit(onSubmit)} className="mt-5 space-y-4">
            <div>
              ...
              <input {...register("name")} type="text" placeholder="Tu nombre" className={fieldCls}/>
              {errors.name && <small className="text-rose-600">{errors.name.message}</small>}
            </div>

            <div>
              <label className={labelCls}>Correo</label>
              <input {...register("email")} type="email" placeholder={`tucorreo@${INSTITUTE_DOMAIN} o personal`} className={fieldCls}/>
              {errors.email && <small className="text-rose-600">{errors.email.message}</small>}
            </div>

            <div>
              <label className={labelCls}>Teléfono</label>
              <input {...register("phone")} type="tel" placeholder="Tu teléfono" className={fieldCls}/>
            ...
            {!internal && (
              <div>
                ...
                <input {...register("school")} type="text" placeholder="Nombre del colegio" className={fieldCls}/>
              </div>
            )}
            ...
            <button
              ...
              type="submit"
              className={`w-full sm:w-auto inline-flex items-center rounded-xl px-4 py-2 font-semibold ${
          ...
          {msg && (
            <div className={`mt-4 rounded-xl px-3 py-2 ${msg.ok ? "bg-emerald-50 text-emerald-700 border border-emerald-200" : "bg-rose-50 text-rose-700 border border-rose-200"}`}>
              {msg.text}
            </div>
          )}
          ...
          {resp && (
            <div className="mt-4 rounded-2xl border border-slate-200 p-4">
              ...
              {resp.qr && (
                <p>
                  Tu QR:{" "}
                  <a href={resp.qr} target="_blank" rel="noreferrer" className="text-umgBlue underline">
                    {resp.qr}
                  </a>
                </p>
              )}
              {resp.emailPreview && (
                <p>
                  ...
                  <a href={resp.emailPreview} target="_blank" rel="noreferrer" className="text-umgBlue underline">
                    Abrir
                  </a>
                </p>
              )}
              {resp.emailMode && (
                <p className="text-xs text-slate-500">
                  Modo de correo: {resp.emailMode}{resp.emailError ? ` (error: ${resp.emailError})` : ""}
                </p>
              )}
            </div>
          )}
