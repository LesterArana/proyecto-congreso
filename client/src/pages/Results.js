...
export default function Results() {
  ...
  const yearsFromData = useMemo(() => {
    const ys = new Set();
    for (const w of items) if (w.activity?.date) ys.add(new Date(w.activity.date).getFullYear());
  ...
  async function load() {
    ...
    try {
      ...
      if (!res.data?.items?.length) setMsg({ ok: true, text: "No hay ganadores publicados aún." });
    } catch {
  ...
  useEffect(() => { load(); }, [year]);       // recarga por año
    ...
    <div className="min-h-screen bg-umgBlue text-white">
      <div className="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        <div className="bg-white text-slate-800 rounded-2xl shadow-soft border border-white/20 p-6">
          ...
          {msg && (
            <div className={`mb-4 rounded-xl px-3 py-2 ${msg.ok ? "bg-emerald-50 text-emerald-700 border border-emerald-200" : "bg-rose-50 text-rose-700 border border-rose-200"}`}>
              {msg.text}
            </div>
          )}
