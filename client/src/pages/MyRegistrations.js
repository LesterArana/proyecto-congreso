    ...
    const results = await Promise.all(
      registrations.map(async (r) => {
        try {
          ...
          return { ...r, diplomaUrl: dres?.data?.downloadUrl || null, diploma: dres?.data?.diploma || null };
        } catch (err) {
    ...
    <div className="min-h-screen bg-umgBlue text-white">
      <div className="max-w-3xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        <div className={cardCls}>
          ...
          <p className="text-slate-600">Ingresa tu correo para ver tus actividades, QR y diplomas disponibles.</p>
              ...
              {data.registrations.map((r) => (
                <div key={r.id} className="rounded-2xl border border-slate-200 p-4">
                  <div className="flex flex-col sm:flex-row justify-between gap-3">
                    <div>
                      <div className="font-semibold">{r.activity.kind}: {r.activity.title}</div>
                      <div className="text-slate-600">{new Date(r.activity.date).toLocaleString()}</div>
                      <div className="text-sm mt-1">Estado: <b>{r.status}</b> — Reg. #{r.id}</div>
                    </div>
                    <div className="sm:text-right flex flex-wrap gap-2 items-center">
                      {r.qr ? (
                        <a href={r.qr} target="_blank" rel="noreferrer" className="inline-flex items-center rounded-xl bg-emerald-600 text-white px-3 py-1.5 hover:brightness-105">Ver QR</a>
                      ) : (
                        <span className="text-sm text-slate-500">Sin QR</span>
                      )}
                      {r.diplomaUrl ? (
                        <a href={r.diplomaUrl} target="_blank" rel="noreferrer" className="inline-flex items-center rounded-xl bg-violet-600 text-white px-3 py-1.5 hover:brightness-105 whitespace-nowrap">
                          Descargar diploma
                        </a>
                      ) : (
                        <span className="text-sm text-slate-500 whitespace-nowrap">Diploma: aún no disponible</span>
                      )}
                    </div>
                  </div>
                </div>
